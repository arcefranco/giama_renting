        ...
        <button
          onClick={() => {
            if (!form["anio"] && !form["mes"]) {
              window.open(`${import.meta.env.VITE_BASENAME}vehiculos/ficha/${data.data.vehiculo}`, '_blank')
            } else {
              window.open(`${import.meta.env.VITE_BASENAME}vehiculos/ficha/${data.data.vehiculo}/${form["anio"]}/${form["mes"]}`, '_blank')
            }
          }
          }
