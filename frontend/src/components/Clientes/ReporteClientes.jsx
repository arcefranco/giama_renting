...
const ReporteClientes = () => {
  ...
  const renderImagenesCell = (data) => {
    return (
      <button
        onClick={() => window.open(`${import.meta.env.VITE_BASENAME}clientes/imagenes/${data.data.id}`, '_blank')}
        style={{ color: '#1976d2', textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer' }}
      >
        Ver imágenes
      </button>
  ...
  const renderImprimirCell = (data) => {
    return (
      <button
        onClick={() => window.open(`${import.meta.env.VITE_BASENAME}clientes/actualizar/${data.data.id}?imprimir=true`, '_blank')}
